function [mean_ex_gauss,sigma_ex_gauss,tau_ex_gauss,X,chiSquare,rt_go_omission_count,rt_go_commission_count,mean_rt_go,sd_rt_go,mean_rt_go_omission_replace,mean_ssd,ssd_range,mean_incorr_stop_trial_rt,race_model_assumption_met,p_respond_given_stop,ssrt_integration,ssrt_mean] = sst_master(filename_SST)
...
if any(go_trial_omissions_log,'all') % check if go rt's contain omissions (no response) 
...
sd_rt_go = std(rt_go_trial) ; %calculate sd rt on all go trials (without replacement for omissions)
...
addpath(genpath('/Volumes/LaCie/Ex_rTMS_study/ex_rtms_code/cognitive/sst/exgauss-master/')) ; %add ex gauss folder w/functions to path

% Select Go RT's above 150ms 
rt_go_trial_2 = rt_go_trial(rt_go_trial > 150); % selected RT distribution corresponding to genuine go responses
...
ssd_range = max(ssd_change(2:end)) - min(ssd_change(2:end)) ; %ssd range (max - min)
...
p_respond_given_stop = total_stop_trial_incorr/ total_ss_trial ; %calculate the percentage of incorrect stop trials (put into output as QC)
...
%compare mean RT go to mean RT unsuccessful stop trials 
% (RT on unsuccessful stop trials should be faster than mean RT go)
% (1 = assumption met, 0 = not met)
race_model_SSRT_assumption_check = mean_rt_go > mean_incorr_stop_trial_rt ;
if race_model_SSRT_assumption_check == 1
    race_model_assumption_met = 'yes, assumption met' ;
else race_model_assumption_met = 'no, assumption not met' ;
end
