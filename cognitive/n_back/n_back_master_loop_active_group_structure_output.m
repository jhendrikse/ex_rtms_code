...
% Condition = {'wk1_llpc';'wk2_sma'} - This was replaced for pathIn_mod - allowing use of wildcard character for stimulation conditions 
...
for x = 1:length(ActiveID)
    ...
    for y = 1:length(Condition)
        ...
        for z = 1:length(timePoint)
            ...
            [Active_data.(ActiveID{x}).(Condition{y}).(timePoint{z}).hits,Active_data.(ActiveID{x}).(Condition{y}).(timePoint{z}).misses,Active_data.(ActiveID{x}).(Condition{y}).(timePoint{z}).fAlarm,Active_data.(ActiveID{x}).(Condition{y}).(timePoint{z}).hitsRT,Active_data.(ActiveID{x}).(Condition{y}).(timePoint{z}).dPrime] = n_back_master(filename_nback);
