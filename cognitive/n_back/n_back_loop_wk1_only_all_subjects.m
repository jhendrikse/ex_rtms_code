...
for x = 1:length(ID)
    ...
    Condition = {pathIn_mod(1).name};
        ...
        for y = 1:length(timePoint)
            ...
            [Data.(ID{x}).Condition.hits(y,1),Data.(ID{x}).Condition.misses(y,1),Data.(ID{x}).Condition.fAlarm(y,1),Data.(ID{x}).Condition.hitsRT(y,1),Data.(ID{x}).Condition.dPrime(y,1)] = n_back_master(filename_nback);
        end
end