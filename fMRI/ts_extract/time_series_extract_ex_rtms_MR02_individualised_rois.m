function [] = time_series_extract_ex_rtms_MR02_individualised_rois
...
rois_dir = ['/projects/kg98/Josh/BIDS_data/MR02/derivatives/fmriprep/',subject{1,z},'/rois/'];
...
savefile = (['roi_ts_individual','_',roifiles(i).name(1:end-4)]) ;
save (savefile,'roi_ts') ;
system(['mv ',savefile,'.mat',' individual_roi']) ;

% concatenate roi time series and nuisance regressors
R = [roi_ts n_reg] ; 

savefile = (['spm_regs_individual','_',roifiles(i).name(1:end-4)]) ; % create variable savefile containing roi time series columns & 8 nuisance regressors columns
...
thespmdir = ([epidir,'individual_roi/','spm','_',roifiles(i).name(1:end-4)]) ;

% Define input directory for time series and regressor file
roi_ts_regs_dir = ([epidir,'individual_roi/']) ;
