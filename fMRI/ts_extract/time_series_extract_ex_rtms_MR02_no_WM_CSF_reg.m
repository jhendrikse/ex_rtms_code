function [] = time_series_extract_ex_rtms_MR02_no_WM_CSF_reg
...
addpath('/projects/kg98/Josh/code/')
...
rois_dir = '/projects/kg98/Josh/BIDS_data/MR01/ROIs/';
...
% select x,y,z,pitch,roll,yaw columns and save as R 
n_reg = [nr.X nr.Y nr.Z nr.RotX nr.RotY nr.RotZ];
...
% tic;
%  
% savefile = (['roi_ts','_',roifiles(i).name(1:end-4)]) ;
% save (savefile,'roi_ts') ;

load((['roi_ts','_',roifiles(i).name(1:end-4),'.mat']),'roi_ts')

% concatenate roi time series and nuisance regressors
R = [roi_ts n_reg] ; 

savefile = (['spm_regs_w_o_csf_wm','_',roifiles(i).name(1:end-4)]) ; % create variable savefile containing roi time series columns & 8 nuisance regressors columns
...
thespmdir = ([epidir,'spm_w_o_csf_wm','_',roifiles(i).name(1:end-4),'/']) ;
...
first_level_ex_rtms(thespmdir,'scans',TR,input_epi_file,N,[epidir,savefile,'.mat'],[t1dir,t1name]) ; % run first level analysis
