function [] = time_series_extract_ex_rtms_ind_rois_stim_site_brain_site
...
%subject = {'sub-DJ03','sub-006','sub-007','sub-008','sub-009','sub-010','sub-011','sub-012','sub-015','sub-CR38',
...
rois_dir = ['/projects/kg98/Josh/BIDS_data/MR01/derivatives/fmriprep/',subject{1,z},'/rois/stim_site_brain_site_5mm_roi/'];
...
system(['mkdir stim_site_brain_site_5mm_roi']) ;
...
savefile = (['roi_ts_stim_site_brain_site_5mm','_',roifiles(i).name(1:end-4)]) ;
save (savefile,'roi_ts') ;
system(['mv ',savefile,'.mat',' stim_site_brain_site_5mm_roi']) ;

% concatenate roi time series and nuisance regressors
R = [roi_ts n_reg] ; 

savefile = (['spm_regs_stim_site_brain_site_5mm','_',roifiles(i).name(1:end-4)]) ; % create variable savefile containing roi time series columns & 8 nuisance regressors columns
...
thespmdir = ([epidir,'stim_site_brain_site_5mm_roi/','spm_brain_site_5mm','_',roifiles(i).name(1:end-4)]) ;

% Define input directory for time series and regressor file
roi_ts_regs_dir = ([epidir,'stim_site_brain_site_5mm_roi/']) ;
