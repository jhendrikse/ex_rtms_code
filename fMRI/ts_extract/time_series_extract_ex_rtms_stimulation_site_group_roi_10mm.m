function [] = time_series_extract_ex_rtms_stimulation_site_group_roi_10mm
...
rois_dir = '/projects/kg98/Josh/BIDS_data/MR01/ROIs/stimulation_site/group/10mm_cortex/';
...
epidir = ['/projects/kg98/Josh/BIDS_data/MR02/derivatives/fmriprep/',subject{1,z},'/func/'] ; % directory containing epi 4d files smoothed by fmriprep
...
% t1 directory 
t1dir = ['/projects/kg98/Josh/BIDS_data/MR02/derivatives/fmriprep/',subject{1,z},'/anat/'];
...
system(['mkdir stim_site_group_roi_10mm_cortex']) ;
...
savefile = (['roi_ts_stim_site_group_mni','_',roifiles(i).name(1:end-4)]) ;
save (savefile,'roi_ts') ;
system(['mv ',savefile,'.mat',' stim_site_group_roi_10mm_cortex']) ;

% concatenate roi time series and nuisance regressors
R = [roi_ts n_reg] ; 

savefile = (['spm_regs_stim_site_group_mni','_',roifiles(i).name(1:end-4)]) ; % create variable savefile containing roi time series columns & 8 nuisance regressors columns
...
thespmdir = ([epidir,'stim_site_group_roi_10mm_cortex/','spm','_',roifiles(i).name(1:end-4)]) ;

% Define input directory for time series and regressor file
roi_ts_regs_dir = ([epidir,'stim_site_group_roi_10mm_cortex/']) ;
...
% Save time stamps
% save times_individual_roi times
% movefile times_individual_roi.mat individual_roi ;
